...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = generateCacheKey(`shl-players-points-${count}`);

    const data = await getCachedData(cacheKey, async () => {
      const url = `https://www.shl.se/api/statistics-v2/stats-info/players_point?count=${count}&ssgtUuid=iuzqg7dqk9&provider=statnet&state=active&moduleType=summary`;
      ...
      if (teamCode) {
        domainData = {
          ...
          stats: domainData.stats.filter(
            (player) => player.info.team.code === teamCode,
          ),
        };
      }
    ...
    console.error('Error fetching SHL player stats:', error);
