import { TeamStats } from "@/app/types/domain/standings";
import {League} from "@/app/types/domain/league";

export const getTeamCode = (team: TeamStats): string =>
    team.info.code;

export const getTeamName = (team: TeamStats): string =>
    team.info.short;

export const getTeamLogo = (team: TeamStats): string =>
    team.info.logo;

export const getRankDisplay = (rank: number | null): string =>
    rank === null ? '-' : rank.toString();

export const getRankBorderClass = (league: League, tablePosition: number, totalTeams: number): string => {
    if (league === 'shl') {
