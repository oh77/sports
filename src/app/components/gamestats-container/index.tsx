...
export const GameStatsContainer: React.FC<HomeAwayContainerProps> = ({ allGames, currentGame }) => {
  ...
  const homeOnlyGames = React.useMemo(() =>
    finishedGames.filter(g => g.homeTeamInfo.teamInfo.code === homeTeamCode),
    [finishedGames, homeTeamCode]
  );

  const awayOnlyGames = React.useMemo(() =>
    finishedGames.filter(g => g.awayTeamInfo.teamInfo.code === awayTeamCode),
    [finishedGames, awayTeamCode]
  );

  const homeLastFiveGames = React.useMemo(() => {
    const homeTeamGames = finishedGames.filter(g =>
  ...
  const awayLastFiveGames = React.useMemo(() => {
    const awayTeamGames = finishedGames.filter(g =>
  ...
  return (
      <div className="rounded-lg shadow p-3" style={{ backgroundColor: 'rgba(255, 255, 255, 0.6)' }}>
          <div className="grid grid-cols-12">
              <div className="col-span-5">
                  <GameStats games={homeOnlyGames} teamCode={homeTeamCode} />
              </div>
              <div className="col-span-2 mt-3.5">
                  <div className="text-xs text-gray-700 mb-1 ml-1 text-center">Hemma / Borta</div>
              </div>
              <div className="col-span-5">
          ...
          <div className="grid grid-cols-12">
              ...
              <div className="col-span-2 mt-3.5">
                  <div className="text-xs text-gray-700 mb-1 ml-1 text-center">5 senaste</div>
              </div>
              <div className="col-span-5">
