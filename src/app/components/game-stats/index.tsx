...
function aggregateStats(games: GameInfo[], teamCode: string): AggregatedStats {
  ...
  for (const g of games) {
    ...
    if (typeof teamGoals === 'number' && typeof oppGoals === 'number') {
      ...
      if (won) {
        if (wentOT) OTW++; else W++;
      } else if (teamGoals < oppGoals) {
        if (wentOT) OTL++; else L++;
      }
    }
  }
...
export const HomeAwayStats: React.FC<HomeAwayStatsProps> = ({ games, teamCode }) => {
  const stats = React.useMemo(() => aggregateStats(games, teamCode), [games, teamCode]);
        ...
        <table className="w-full">
            <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    >
                        ...
                        <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-900 text-center">
                            <span className="hidden md:table-cell">{stats.OTW} | {stats.OTL}</span>
