...
function transformMatchToGame(
  match: CHLMatch,
  teamsMap?: Map<string, CHLTeamInfo>,
): CHLGame {
...
export async function getUpcomingGames(): Promise<CHLGame[]> {
  try {
    ...
    const upcomingMatches = data.data.filter(
      (match) =>
        match.status === 'not-started' && new Date(match.startDate) > now,
    );
    ...
    return upcomingMatches
      .slice(0, 3)
      .map((match) => transformMatchToGame(match, teamsMap));
  } catch (error) {
...
export async function getGamesByDate(date: string): Promise<CHLGame[]> {
  try {
    ...
    return gamesForDate.map((match) => transformMatchToGame(match, teamsMap));
  } catch (error) {
...
export async function getRecentGames(): Promise<CHLGame[]> {
  try {
    ...
    const finishedMatches = data.data.filter(
...
export async function getAllGames(): Promise<CHLGame[]> {
  try {
    ...
    return data.data.map((match) => transformMatchToGame(match, teamsMap));
  } catch (error) {
...
export async function getAllUpcomingGames(): Promise<CHLGame[]> {
  try {
    ...
    return upcomingMatches.map((match) =>
      transformMatchToGame(match, teamsMap),
    );
  } catch (error) {
...
export function getNextGameDay(games: CHLGame[]): string | null {
