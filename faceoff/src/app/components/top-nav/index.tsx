...
export function TopNav({ league, season }: TopNavProps) {
  ...
  const base = leagueBasePath(league, season);
    ...
    <>
      ...
      <header className="sticky top-0 z-30 border-b border-line-strong bg-bg/95 backdrop-blur">
        <div className="relative mx-auto flex h-[60px] max-w-7xl items-center px-4 md:px-6">
          {/* League logo + name (opens league switcher) */}
          <div className="relative pr-4 md:pr-8">
            <button
              ...
              aria-label="Byt liga"
            >
              <span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-md bg-surface">
                <Image
                  src={logo}
                  alt=""
                  width={28}
            ...
            {menuOpen && (
              <>
                ...
                />
                <div className="absolute left-0 top-full z-50 mt-2 min-w-[180px] overflow-hidden rounded-lg border border-line bg-surface py-1.5 shadow-2xl">
